...
Select-AzureSubscription -SubscriptionName "ssp-apim-development"
...
$secureStringPassword = ConvertTo-SecureString "P@ssw0rd" -AsPlainText -Force
Add-AzureRmApplicationGatewaySslCertificate -ApplicationGateway $agw -Name ($ApimName + "-SSLCertificate") `
    -CertificateFile $tmpFile.FullName -Password $secureStringPassword
...
$authCert = New-AzureRmApplicationGatewayAuthenticationCertificate  -Name ($ApimName + "-AuthCertificate") `
    -CertificateFile $tmpFileAuth.FullName 
 Set-AzureRmApplicationGatewayAuthenticationCertificate -ApplicationGateway $agw -
...
# initialize front end port
 ...
 $Frontendport = Get-AzureRmApplicationGatewayFrontendPort -ApplicationGateway $agw


#Remove-AzureRmApplicationGatewayBackendHttpSettings -ApplicationGateway $agw -Name ($ApimName + "-APIListener")
